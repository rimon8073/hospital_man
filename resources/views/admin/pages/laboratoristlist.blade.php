...
<div style="height: 900px" class="box round first grid">
    ...
    <a href="{{URL::to('laboratoristlist')}}"><span style="border-bottom: 2px solid black" class="btn btn-gray">Laborotoryist List</span></a>
...
@endif
					<tbody>
            @php $i=0; @endphp
            @foreach ($laboratoristlist as $laboratory)
              @php $i++ @endphp
			<tr class="odd gradeX">
				<td>{{ $i}}</td>
 
  			<td>
                <img src="images/employee/{{$laboratory->emp_image}}" alt="IMG" style="width:100px;height:50px;">
                </td>
              <td>{{ $laboratory->emp_name }}</td>
							<td>{{ $laboratory->emp_email }}</td>
							<td>{{ $laboratory->emp_mobile }}</td>
          <td><a class="btn btn-xs btn-success" href="{{Route('laboratoristview')}}/{{ $laboratory->id }}"><i class="fa fa-eye"></i></a> <a class="btn btn-xs btn-primary" href="{{Route('laboratoristedit')}}/{{$laboratory->id}}"><i class="fa fa-edit"></i></a> <a class="btn btn-xs btn-danger" onclick= "return confirm('Are you sure to delete this row !')" href="{{Route('laboratoristdelete')}}/{{$laboratory->id}}"><i class="fa fa-trash"></i></a></td>
